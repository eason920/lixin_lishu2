...
export default {
  ...
  methods: {
    prevBtn() {
      this.$refs.swiper3.$swiper.slidePrev()
    },
    nextBtn() {
      this.$refs.swiper3.$swiper.slideNext()
    },
    fnDotChange(i) {
      let eq = i - 1
      if (eq >= 3) {
      ...
      this.$refs.swiper3.$swiper.slideTo(eq)
    },
    fnCCheck(i) {
      if (i === this.nCurrent) {
        this.bOn = !this.bOn
